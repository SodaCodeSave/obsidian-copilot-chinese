...
export default class EmbeddingManager {
  ...
  async getEmbeddingsAPI(): Promise<Embeddings> {
    ...
    // All models are accessible to all users
    // Removed plus-exclusive and believer-exclusive model checks
  ...
  private async getEmbeddingConfig(customModel: CustomModel): Promise<any> {
    ...
    const providerConfig: {
      ...
      [EmbeddingModelProviders.MODELSCOPE]: {
        modelName,
        apiKey: await getDecryptedKey(customModel.apiKey || settings.modelscopeApiKey),
        batchSize: getSettings().embeddingBatchSize,
        configuration: {
